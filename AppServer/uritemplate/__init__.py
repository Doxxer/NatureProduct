...
VAR = re.compile(r"^(?P<varname>[^=\+\*:\^]+)((?P<explode>[\+\*])|(?P<partial>[:\^]-?[0-9]+))?(=(?P<default>.*))?$",
                 re.UNICODE)
...
def _tostring(varname, value, explode, operator, safe=""):
    ...
    if type(value) == type({}):
        ...
        if explode == "+":
            return ",".join(
                [varname + "." + urllib.quote(key, safe) + "," + urllib.quote(value[key], safe) for key in keys])
        else:
...
def _tostring_path(varname, value, explode, operator, safe=""):
    ...
    if type(value) == type([]):
        ...
        if explode == "+":
            return joiner.join(
                [varname + "." + urllib.quote(key, safe) + joiner + urllib.quote(value[key], safe) for key in keys])
        elif explode == "*":
    ...
    if type(value) == type([]):
        ...
        if explode == "+":
            ...
            return varprefix + ",".join(
                [urllib.quote(key, safe) + "," + urllib.quote(value[key], safe) for key in keys])
    else:
        if value:
